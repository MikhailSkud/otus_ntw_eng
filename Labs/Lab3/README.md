...
### [Часть 2: Проверить получение SLAAC адресов с маршрутизатора R1](#часть-2-проверить-получение-slaac-адресов-с-маршрутизатора-r_1)

### [Часть 3: Настроить Stateless DHCPv6 сервер на маршрутизаторе R1](#часть-3-настроить-stateless-dhcpv6-сервер-на-маршрутизаторе-r_1)

### [Часть 4: Настроить Stateful DHCPv6 сервер на маршрутизаторе R1](#часть-4-настроить-stateful-dhcpv6-сервер-на-маршрутизаторе-r_1)

### [Часть 5: Настроить DHCPv6 Relay на маршрутизаторе R2](#часть-5-настроить-dhcpv6-relay-на-маршрутизаторе-r_2)

### `Инструкции:`

### Часть 1: Построить схему сети произвести базовую конфигурацию устройств

#### Шаг 1: Подключаем устройства так, как показано в топологии

#### Шаг 2: Производим базовую настройку коммутаторов [S1](/Labs/Lab3/ipv6_config/S1.txt) и [S2](/Labs/Lab3/ipv6_config/S2.txt)

#### Шаг 3: Производим базовую настройку маршрутизаторов [R1](/Labs/Lab3/ipv6_config/R1.txt) и [R2](/Labs/Lab3/ipv6_config/R2.txt)

#### Шаг 4: Настраиваем интерфейсы и маршрутизацию для обоих маршрутизаторов

Интерфейсы на маршрутизаторе R1 настроены:   

![alt text](image-13.png)

Интерфейсы на маршрутизаторе R2 настроены:    

![alt text](image-14.png)

Статический маршрут на интерфейс Gi0/0 маршрутизатора R2 настроен:   

![alt text](image-15.png)

Статический маршрут на интерфейс Gi0/0 маршрутизатора R1 настроен:    

![alt text](image-16.png)

Проверим, что маршрутизация работает:

![alt text](image-17.png)

### Часть 2: Проверить получение SLAAC адресов с маршрутизатора R_1

Проверяем получение адреса SLAAC на PC-A:

![alt text](SLAAC_PCA.jpg)

Проверяем получение адреса SLAAC на PC-B:

![alt text](SLAAC_PCB.jpg)

<b>Хостовая часть ipv6 адреса может быть сгенерирована случайным образом или опираясь на MAC адрес интерфейса. Зависит от настроек операционной системы.</b> 

### Часть 3: Настроить Stateless DHCPv6 сервер на маршрутизаторе R_1

Создаем IPv6 DHCP пул:   

![alt text](image-18.png)

Настраиваем интерфейс Gi0/1 с флагом OTHER:   

![alt text](image-19.png)

Добавляется DNS сервер и доменное имя STATELESS.com:

![alt text](image-20.png)

Проверяем доступность интерфейса Gi0/1 маршрутизатора R2:

![alt text](image-21.png)   

### Часть 4: Настроить Stateful DHCPv6 сервер на маршрутизаторе R_1

Создаем IPv6 DHCP пул c сервером DNS и доменным именем STATEFUL.com:   

![alt text](image-22.png)

Настраиваем интерфейс Gi0/0:   

![alt text](image-23.png)

### Часть 5: Настроить DHCPv6 Relay на маршрутизаторе R_2

#### Шаг 1: Проверяем получение SLAAC адреса на PC-B:

![alt text](image-24.png)

#### Шаг 2: Настраиваем маршрутизатор R2 как DHCP ретранслятор:

![alt text](image-27.png)

#### Шаг 3: Пытаемся получить IPv6 адрес с DHCP сервера на PC-B:

![alt text](image-25.png)

Проверяем доступность интерфейса Gi0/1 маршрутизатора R1:
